...
class MyTheme {
  ...
  Future<void> addSystem(Brightness data) async {
    if (data != _system.value) {
      MyLog.info('set system theme to stream: $data');
      _system.add(data);
    }
    if (data == Brightness.dark) {
      ...
      MyLog.warn('unknow system theme $data');
  ...
  Future<void> loadSystem() async {
    try {
      ...
      MyLog.info('load system theme: $name');
      if (name == "dark") {
      ...
      MyLog.warn('load system theme error: $e');
    }
  }
  ...
  Future<void> saveSystem(String? name) async {
    try {
      ...
      if (name == null) {
        ...
        MyLog.info('save system theme: $name');
        await prefs.setString("system_theme", name);
      }
    } catch (e) {
      MyLog.info('save system theme error: $e');
    }
  }
